...
def rnn_step_backward(dnext_h, cache):
  ...
  ##############################################################################
  x, prev_h, Wx, Wh, state = cache

  dstate = (1 - np.tanh(state) ** 2) * dnext_h
...
def rnn_backward(dh, cache):
  ...
  for t in reversed(range(T)):
    dx[:, t, :], dh0, dWx_t, dWh_t, db_t = rnn_step_backward(dh[:, t, :] + dh0, cache_step[t])
    dWx += dWx_t
    dWh += dWh_t
    db += db_t
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  #############################################################################
  next_c, og, ig, fg, og, update, ai, af, ao, au, Wx, x, Wh, prev_h, prev_c = cache

  dog = np.tanh(next_c) * dnext_h
  dnext_c += og * (1 - np.tanh(next_c) ** 2) * dnext_h
  ...
  dau = (1 - np.tanh(au) ** 2) * dupdate
  dao = sigmoid(ao) * (1 - sigmoid(ao)) * dog
  daf = sigmoid(af) * (1 - sigmoid(af)) * dfg
  dai = sigmoid(ai) * (1 - sigmoid(ai)) * dig

  da = np.hstack((dai, daf, dao, dau))
...
def lstm_backward(dh, cache):
  ...
  for t in reversed(range(T)):
    dx_t, dh0, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dh[:, t, :] + dh0, dprev_c, step_cache[t])
    dx[:, t, :] = dx_t
    dWx += dWx_t
    dWh += dWh_t
    db += db_t
