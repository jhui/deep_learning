...
def batchnorm_backward(dout, cache):
  ...
  disqrt_var = np.sum(xmu * dxhat, axis=0)
  dxmu1 = dxhat / sqrt_var

  dvar = - 0.5 * (sample_var + eps) ** (-3./2) * disqrt_var
...
def batchnorm_backward_alt(dout, cache):
  ...
  dx = gamma * (dout - (dbeta + xhat * dgamma) / N ) / sqrt_var
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  filter_size = C * HH * WW
  filter = np.reshape(w, (F, filter_size)).T
  for i in range(H_prime):
    ...
    for j in range(W_prime):
      left = j * stride
      right = left + WW
      data = np.reshape(x_pad[:, :, top:bottom, left:right], (N, filter_size))
      y = data.dot(filter) + b
...
def conv_backward_naive(dout, cache):
  ...
  H_prime = 1 + (DH + 2 * pad - HH) / stride
  W_prime = 1 + (DW + 2 * pad - WW) / stride

  filter_size = C * HH * WW

  dx_pad = np.zeros((N, C, H + 2 * pad, W + 2 * pad))
  filter = np.reshape(w, (F, filter_size))
  x_pad = np.pad(x, ((0,), (0,), (pad,), (pad,)), 'constant')
  dw = np.zeros((F, filter_size))
  for i in range(H_prime):
    top = i * stride
    bottom = top + HH
    for j in range(W_prime):
      ...
      dx_window = dout_ij.dot(filter)
      dx_window = dx_window.reshape(N, C, HH, WW)
      dx_pad[:, :, top:bottom, left:right] += dx_window

      x_window = x_pad[:, :, top:bottom, left:right].reshape(N, C * HH * WW)
      dw += dout_ij.T.dot(x_window)
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(H_prime):
    ...
    for j in range(W_prime):
      ...
      window = x[:, :, top:bottom, left:right].reshape((N * C, pool_height * pool_width))
      max_i = np.argmax(window, axis=1)

      dx_window = dx[:, :, top:bottom, left:right].reshape((N * C, pool_height * pool_width)).T
      dx_window[max_i, range(N * C)] += dout_ij

      dx[:, :, top:bottom, left:right] += dx_window.T.reshape(N, C, pool_height, pool_width)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
