...
lmbda_values = np.logspace(-3.0, -1.0, num=lmbda_steps)
accuracies = []

num_steps = 3001
num_batches = 1

print('Lambda values used', lmbda_values)
for lmbda_value in lmbda_values:
    with tf.Session(graph=graph) as session:
      tf.global_variables_initializer().run()
      print("\nInitialized with regularization value: %.5f use %s batches" % (lmbda_value, str(num_batches)))
...
plt.grid(True)
plt.title("Test accuracy with regularization lambda (1 hidden layer with 1024 nodes) use " +  str(num_batches) + ' batches')
plt.show()