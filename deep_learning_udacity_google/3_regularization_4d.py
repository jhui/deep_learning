...
with graph.as_default():
    ...
    dropout1 = tf.nn.dropout(hidden_layer1, 0.5)
    hidden_layer2 = tf.nn.relu(tf.matmul(dropout1, weights2) + biases2)
    dropout2 = tf.nn.dropout(hidden_layer2, 0.5)
    hidden_layer3 = tf.nn.relu(tf.matmul(dropout2, weights3) + biases3)
    dropout3 = tf.nn.dropout(hidden_layer3, 0.5)
    ...
    valid_prediction = tf.nn.softmax(tf.matmul(valid_h3, weights4) + biases4)
    ...
    test_prediction = tf.nn.softmax(tf.matmul(test_h3, weights4) + biases4)

num_steps = 9001

with tf.Session(graph=graph) as session:
    tf.global_variables_initializer().run()
    print("\nInitialized with decay learning rate and dropout.")
    ...
    print("Completed. Trained with decay learning rate and dropout.")
