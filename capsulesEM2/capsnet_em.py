...
def spread_loss(output, pose_out, x, y, m):
    """
    output: prediction outout (50, 10)
    pose_out: (50, 10, 18)
    x: input (50, 28, 28, 1)
    y: label (50,)
    """

    num_class = int(output.get_shape()[-1]) # 10
    data_size = int(x.get_shape()[1])       # 28

    y = tf.one_hot(y, num_class, dtype=tf.float32) # (50, 10)

    # spread loss
    output1 = tf.reshape(output, shape=[cfg.batch_size, 1, num_class]) # (50, 1, 10)
    y = tf.expand_dims(y, axis=2)    # (50, 10, 1)
    at = tf.matmul(output1, y)       # (50, 1, 1)
    # Compute the spread loss
    loss = tf.square(tf.maximum(0., m - (at - output1)))               # (50, 1, 10)
    loss = tf.matmul(loss, 1. - y)   # (50, 1, 1)
    loss = tf.reduce_mean(loss)      # ()

    # reconstruction loss
    pose_out = tf.reshape(tf.matmul(pose_out, y, transpose_a=True), shape=[cfg.batch_size, -1])  # (50, 18)
    ...
    with tf.variable_scope('decoder'):
        ...
        pose_out = slim.fully_connected(pose_out, data_size * data_size,
                                        trainable=True, activation_fn=tf.sigmoid)                # (50, 784)

        x = tf.reshape(x, shape=[cfg.batch_size, -1])                                            # (50, 784)
        reconstruction_loss = tf.reduce_mean(tf.square(pose_out - x))

    if cfg.weight_reg:     # False
        # regularization loss
        regularization = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
...
def build_arch(input, coord_add, is_train: bool, num_classes: int):
    data_size = int(input.get_shape()[1])  # 28
    ...
    # The paper did not mention any regularization, a L2 regularization is added here
    weights_regularizer = tf.contrib.layers.l2_regularizer(5e-04)
    ...
    with slim.arg_scope([slim.conv2d], trainable=is_train, biases_initializer=bias_initializer, weights_regularizer=weights_regularizer):
        with tf.variable_scope('relu_conv1') as scope:
            output = slim.conv2d(input, num_outputs=cfg.A, kernel_size=[
                                 5, 5], stride=2, padding='VALID', scope=scope, activation_fn=tf.nn.relu) # (50, 12, 12, 32)
            data_size = int(np.floor((data_size - 4) / 2))   # 12
        ...
        with tf.variable_scope('primary_caps') as scope:
            pose = slim.conv2d(output, num_outputs=cfg.B * 16,
                               kernel_size=[1, 1], stride=1, padding='VALID', scope=scope, activation_fn=None) # (50, 12, 12, 8x16=128)
            activation = slim.conv2d(output, num_outputs=cfg.B, kernel_size=[
                                     1, 1], stride=1, padding='VALID', scope='primary_caps/activation', activation_fn=tf.nn.sigmoid) # (50, 12, 12, 8)
            pose = tf.reshape(pose, shape=[cfg.batch_size, data_size, data_size, cfg.B, 16]) # (50, 12, 12, 8, 16)
            activation = tf.reshape(
                activation, shape=[cfg.batch_size, data_size, data_size, cfg.B, 1]) # (50, 12, 12, 8, 1)
            output = tf.concat([pose, activation], axis=4)                                  # (50, 12, 12, 8, 17)
            output = tf.reshape(output, shape=[cfg.batch_size, data_size, data_size, -1])   # (50, 12, 12, 136)
        ...
        with tf.variable_scope('conv_caps1') as scope:
            ...
            with tf.variable_scope('v') as scope:
                votes = mat_transform(output[:, :, :16], cfg.C, weights_regularizer, tag=True) # (1250, 72, 16, 16)
            ...
            pose = tf.reshape(miu, shape=[cfg.batch_size, data_size, data_size, cfg.C, 16]) # (50, 5, 5, 16, 16)
            tf.logging.info('conv cap 1 pose shape: {}'.format(pose.get_shape()))
            activation = tf.reshape(
                activation, shape=[cfg.batch_size, data_size, data_size, cfg.C, 1])   # (50, 5, 5, 16, 1)
            tf.logging.info('conv cap 1 activation after reshape: {}'.format(
                activation.get_shape()))
            output = tf.reshape(tf.concat([pose, activation], axis=4), [              # (50, 5, 5, 272)
        ...
        with tf.variable_scope('conv_caps2') as scope:
            output = kernel_tile(output, 3, 1)                                        # (50, 3, 3, 9, 272)
            data_size = int(np.floor((data_size - 2) / 1))                            # 3
            output = tf.reshape(output, shape=[cfg.batch_size *
                                               data_size * data_size, 3 * 3 * cfg.C, 17]) # (450, 144, 17)
            activation = tf.reshape(output[:, :, 16], shape=[
                                    cfg.batch_size * data_size * data_size, 3 * 3 * cfg.C, 1]) # (450, 144, 1)

            with tf.variable_scope('v') as scope:
                votes = mat_transform(output[:, :, :16], cfg.D, weights_regularizer)  # (450, 144, 16, 16)
                tf.logging.info('conv cap 2 votes shape: {}'.format(votes.get_shape()))

            with tf.variable_scope('routing') as scope:
                miu, activation, _ = em_routing(votes, activation, cfg.D, weights_regularizer) # (450, 1, 16, 16), (450, 16)

            pose = tf.reshape(miu, shape=[cfg.batch_size * data_size * data_size, cfg.D, 16])  # (450, 16, 16)
            tf.logging.info('conv cap 2 pose shape: {}'.format(votes.get_shape()))
            activation = tf.reshape(
                activation, shape=[cfg.batch_size * data_size * data_size, cfg.D, 1])          # (450, 16, 1)
        ...
        with tf.variable_scope('class_caps') as scope:
            with tf.variable_scope('v') as scope:
                votes = mat_transform(pose, num_classes, weights_regularizer)    # (450, 16, 10, 16)
                ...
                coord_add = np.reshape(coord_add, newshape=[data_size * data_size, 1, 1, 2])  # (9, 1, 1, 2)
                coord_add = np.tile(coord_add, [cfg.batch_size, cfg.D, num_classes, 1])       # (450, 16, 10, 2)
                coord_add_op = tf.constant(coord_add, dtype=tf.float32)

                votes = tf.concat([coord_add_op, votes], axis=3)                              # (450, 16, 10, 18)
                tf.logging.info('class cap votes coord add shape: {}'.format(votes.get_shape()))

            with tf.variable_scope('routing') as scope:
                miu, activation, test2 = em_routing(
                    votes, activation, num_classes, weights_regularizer)                      # (450, 1, 10, 18), (450, 10)
                tf.logging.info(
            ...
            output = tf.reshape(activation, shape=[
                                cfg.batch_size, data_size, data_size, num_classes])           # (50, 3, 3, 10)

        output = tf.reshape(tf.nn.avg_pool(output, ksize=[1, data_size, data_size, 1], strides=[
                            1, 1, 1, 1], padding='VALID'), shape=[cfg.batch_size, num_classes]) # (50, 10)
        ...
        pose = tf.nn.avg_pool(tf.reshape(miu, shape=[cfg.batch_size, data_size, data_size, -1]), ksize=[
                              1, data_size, data_size, 1], strides=[1, 1, 1, 1], padding='VALID') #(50, 1, 1, 180)
        pose_out = tf.reshape(pose, shape=[cfg.batch_size, num_classes, 18]) # (50, 10, 18)
