...
class Color():
    def __init__(self, imgsize=256, batchsize=4):
        ...
        self.c_category_dim = 20
        self.c_uniform_dim = 200
        self.c_gaussian_dim = 200

        self.generator_h_dim = 4096
        self.discriminator_h_dim = 4096
        ...
        self.d_loss = self.d_loss_real + self.d_loss_fake + self.q_loss
        ...
        self.g_loss = self.g_loss_fake + self.q_loss
    ...
    def discriminator(self, image, generate_q = False):
        ...
        if generate_q:
            ...
            q = tf.reshape(q_h1, [self.batch_size, -1]) # (N, 65536)
            logits_category = linear(q, self.c_category_dim, 'd_q1_lin') # (N, 20)
            c_category = tf.nn.softmax(logits_category) # (N, 20)
            c_uniform  = linear(q, self.c_uniform_dim, 'd_q2_lin')  # (N, 200)
            c_gaussian = linear(q, self.c_gaussian_dim, 'd_q3_lin') # (N, 200)
    ...
    def generator(self, img_in, c_category, c_uniform, c_gaussian):
        ...
        e7 = self.bn_g(conv2d(lrelu(e6), self.gf_dim * 16, name='g_e7_conv'))  # (N, 2, 2, 1024)

        l1 = tf.reshape(e7, [self.batch_size, -1]) # (N, 4096)
        l2 = tf.concat([l1, c_category, c_gaussian, c_uniform], 1) # (N, 4516)
        l3 = self.bn_g(linear(l2, self.generator_h_dim , 'g_l1_lin'))   # (N, 4096)
        l4 = tf.reshape(l3, [self.batch_size, 2, 2, self.generator_h_dim//4]) # (N, 2, 2, 1024)

        self.d2, self.d2_w, self.d2_b = deconv2d(tf.nn.relu(l4), [self.batch_size, s64, s64, self.gf_dim * 16],
