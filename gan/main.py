...
class Color():
    ...
    def discriminator(self, image, y=None):
        ...
        h3 = lrelu(
            self.d_bn3(conv2d(h2, self.df_dim * 8, stride_h=1, stride_w=1, name='d_h3_conv')))  # (N, 32, 32, 512)
    ...
    def generator(self, img_in):
        ...
        self.d7, self.d7_w, self.d7_b = deconv2d(tf.nn.relu(d6), [self.batch_size, s2, s2, self.gf_dim], name='g_d7',
                                                 with_w=True)  # (N, 128, 128, 64)
        d7 = bn(self.d7)
