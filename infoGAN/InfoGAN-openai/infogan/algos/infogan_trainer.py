...
class InfoGANTrainer(object):
    ...
    def init_opt(self):
        ...
        with pt.defaults_scope(phase=pt.Phase.train):
            z_var = self.model.latent_dist.sample_prior(self.batch_size) # (128, 74)
            fake_x, _ = self.model.generate(z_var)
            real_d, _, _, _ = self.model.discriminate(input_tensor)
            fake_d, _, fake_reg_z_dist_info, _ = self.model.discriminate(fake_x) # (128,), (128, 10)

            reg_z = self.model.reg_z(z_var)  # (128, 12)
            ...
            if len(self.model.reg_disc_latent_dist.dists) > 0:
                disc_reg_z = self.model.disc_reg_z(reg_z)   # (128, 12) -> (128, 10)
                disc_reg_dist_info = self.model.disc_reg_dist_info(fake_reg_z_dist_info) # (128, 10)
